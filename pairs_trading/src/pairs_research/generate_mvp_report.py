...
def build_report(base_dir: Path) -> tuple[str, list[str]]:
    ...
    lines.extend(
        [
            ...
            "## Key Finding",
            "",
            f"The selected walk-forward test returned {pct(walk.get('total_return'))}, with Sharpe {num(walk.get('sharpe'))} and maximum drawdown {pct(walk.get('max_drawdown'))}. This sample does not establish durable profitability; the selected pairs and results above describe only this run.",
            "",
            "## MVP Limitations",
            "",
