...
### Baseline execution evidence — 2026-09-23

The complete suite passes **122 tests**. The official experiment ran from committed harness/input revision `2dda568`; a second run using a separately installed wheel outside the repository produced an identical manifest and all seven artifacts byte-for-byte. Both manifests confirm clean source/inputs. All 15 test pair accounts reconcile daily P&L to completed trades, ordinary fills occur after signal dates, and final positions are flat. The figure was rendered and visually checked.

Experiment 001 reports 9/15 positive pair tests and a 0.48% median pair return over 2025, with all losses retained. These are independent $100,000 pair accounts with $10,000 entry budgets, not aggregate portfolio returns. The [generated report](../research/experiments/001_rolling_ols_baseline/report.md) and [manifest](../research/experiments/001_rolling_ols_baseline/manifest.json) are the authoritative outputs; no parameter was changed in response to the test results.
